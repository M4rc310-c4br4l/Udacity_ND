...
#        distance_from_target = np.tanh(sum(abs(self.sim.pose[:3] - np.float32(self.target_pos))) / 3)
...
#        euler_change = np.tanh(sum(abs(old_angular_v - self.sim.angular_v)))

        velocity_change = sigmoid(sum(abs(old_v - self.sim.v)))
#        velocity_change = np.tanh(sum(abs(old_v - self.sim.v)))
